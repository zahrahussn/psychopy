...
fileName = params['participant ID']+'_MotionExperiment'
dataFile = open(fileName+'.csv', 'w')#a simple text file with 'comma-separated-values'
dataFile.write('coherence, direction, corresp, subjectResp, accuracy, incorrect RT\n') 
...
for thisTrial in trials:
    ...
    nDots=100, fieldShape='circle', fieldPos=(0.0, 0.0), fieldSize=10, dotSize=0.02, opacity=1, contrast=1, 
    dotLife=5,  # number of frames for each dot to be drawn
    signalDots='same',  # are signal dots 'same' on each frame? (see Scase et al)
    noiseDots='direction',  # do the noise dots follow random- 'walk', 'direction', or 'position'
    speed=0.05, coherence=trials.thisTrial.coherence)
#        
    #instructions
    win.mouseVisible=False
...
#    circleRight.draw()
    ...
    # collect response
        allKeys = event.getKeys(keyList=['escape','up','down'],timeStamped = clockRT)
